...
| `skills/` | Skills — `/jira` (Jira Cloud via `mcp-atlassian` — create / edit / search / transition / comment / link Epics, Tasks, Bugs) and `/design` (guided UI design; lives here rather than in `commands/` so it takes precedence over Claude Code's bundled `design` skill of the same name — its knowledge files sit next to `SKILL.md`). Plus one command-bound resource bundle loaded by path (no `SKILL.md`): `architecture-review/` (depth/locality method for `/architecture-review`). |
