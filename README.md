...
| `settings.json` → `skillOverrides` | Switches off the Claude Code bundled skills the starter replaces or does not want auto-triggered (`init`, `design`, `dataviz`, `run`, `simplify`, the `artifact-*` trio, `claude-in-chrome`, …). Kept on: `code-review` and `security-review` (the quality pipeline calls them), `loop`, `schedule`, `claude-api`, `update-config`. Undocumented key, verified on Claude Code 2.1.257 (2026-09-16); values `off` / `user-invocable-only` / `name-only`. |
