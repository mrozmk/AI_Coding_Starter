...
| Path | Purpose |
|------|---------|
| `commands/` | Slash commands — `/setup:start` (the guided bootstrap — run it first), `/brainstorm`, `/plan-feature`, `/execute`, `/harness:gates-verify-implementation`, `/harness:gates-design-quality-check`, `/harness:gates-check-quality`, `/check-implementation`, `/quick-change`, `/deep-review`, `/design`, `/architecture-review`, `/orchestrate`, `/commit`, `/push`, `/pull`, `/release`, `/start-task`, `/analysis`, `/simply`, `/handoff`, `/prime`, `/prime-ba`, `/prime-qa`, `/qa-verify`, `/recon`, `/setup:create-PRD`, `/maintain:refresh-brief`, `/setup:create-backlog`, `/setup:stack-research`, `/setup:create-CLAUDE_MD`, `/setup:map-codebase`, `/maintain:sync-from-starter`, `/test-e2e`, `/maintain:cleanup-workflow`, `/retro`, `/setup:createwikillm` |
| *(no `agents/`)* | Every sub-agent now ships in the `harness` plugin: the `/orchestrate` pipeline agents (`harness:orchestrator-executor`, `-executor-hard`, `-refiner`, `-verifier`, `-committer`, `-designer`) and `harness:documentation-manager` since 0.3.0, the `/qa-verify` verifiers (`qa-contract`, `qa-runtime-ui`) since 0.4.0. The starter keeps no agent files of its own. |
...
## Harness plugin — planning / git / execution / product / QA release (0.4.0)

The workflow's command core is authored once under `harness-source/` and built into two natively installable plugins — `packages/claude` for Claude Code and `packages/codex` for Codex CLI — with fail-closed cross-model review in both directions (Claude author → Codex reviewer, Codex author → Claude reviewer). Five stages ship: **planning** (`prime`, `brainstorm`, `plan-feature`, `setup-start`, `handoff`), **git** since 0.2.0 (`commit`, `push`, `pull`, `release`, `pr-create`, `start-task`), **execution** since 0.3.0 (`execute`, `check-implementation`, `quick-change`, `deep-review`, `orchestrate`, `recon`, `analysis`, `design`, `test-e2e`, `architecture-review`, the three `gates-*` skills, the `orchestrator-*` agents and a supervised Codex executor), and since 0.4.0 both **product** (`create-prd`, `refresh-brief`, `create-backlog`, `stack-research`, `prime-ba`) and **QA** (`qa-verify`, `prime-qa` plus the `qa-contract` / `qa-runtime-ui` verifiers) — behind two new capability groups, `product` and `qa` — together with the session peripherals `retro` and `simply` and the Atlassian integrations `jira` and `confluence`.

In a project each bare command (`/prime`, `/execute`, …) is a generated wrapper that routes to the plugin skill; `/harness:setup-start` installs them. Namespaced commands get no wrapper and are invoked directly as `/harness:<skill>` — that includes `jira` and `confluence`. What stays in the project as ordinary starter files: the bootstrap and maintenance commands (`setup/*`, `maintain/*`), the `pr-comments` skill (permission-pinned), the convention templates, the `Simply` output style, the `map-codebase` workflow, `settings.json`, `.mcp.json` and the Bash hooks. Migration steps for a project on 0.3.0 are in `docs/harness/installation.md → Migrate 0.3.0 → 0.4.0 (downstream, by hand)`; a project still on 0.2.0 applies that section's predecessor first.
...
node scripts/build-harness.mjs --export dist/harness-0.4.0   # bundle; harness-release.json says whether live evidence for these bytes ships
