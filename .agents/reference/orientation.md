# Orientation steps — quick-mode context load

> Reference copy of the legacy `/prime` procedure. `/prime` itself is now a wrapper for the plugin skill `harness:prime`; this file exists for spawned reviewers and executors (Codex children, subagents) that cannot run a slash command or a plugin skill and are told to *read and follow* the steps instead. Quick mode = sections 1-3, 6-7. Keep in step with `harness-source/skills/prime/SKILL.md` when the plugin changes what it loads.


## Modes

- **Quick mode** (default). Loads only essentials: `CLAUDE.md`, `index.md`, `project-brief.md`, `architecture.md`, plus listings of plans/specs/reference and git state. Suitable for most sessions — additional memory files load on-demand via `index.md → When to Read`.
- **Full mode**. Adds `patterns.md`, `decisions.md`, `api.md`, `errors.md`, all populated `domain/*` files, and as much of `.agents/reference/` as fits a hard context budget (Step 5). **`.agents/specs/` is never auto-read** — it is listed, and the spec for the current task is read on demand. Use when returning to a project after a long break or starting a deep multi-area task.
