...
| Directory structure, file roles, naming rules | [architecture.md](architecture.md) | regenerated by `/harness:create-rules` |
...
| Bootstrapping a fresh clone / choosing which capability groups to keep | the project profile (written by `/harness:setup-start`; `.claude/project-profile.json.example` documents the legacy schema) |
...
> **Project-specific rows** are appended here by `/harness:create-rules` once detected (e.g. AI layer → specific domain files, i18n module → its own row). Keep them in this table, not in `CLAUDE.md`.
