// Deliberately small JSON-Schema subset validator (type, required, properties, additionalProperties,
// enum, const, items, minItems, minimum, minLength, pattern, oneOf/anyOf, local $ref). Vendored here
// because the runner ships byte-identical to every downstream and may import node: builtins only.
// Returns `path: message` strings; empty means valid.
...
export function validate(schema, value, at = '$', root = schema) {
  ...
  if (typeof value === 'number' && schema.minimum !== undefined && value < schema.minimum) fail(`below minimum ${schema.minimum}`);
