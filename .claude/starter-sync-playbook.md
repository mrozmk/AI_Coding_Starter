...
- `.agents/reference/qa-evidence-families.md` — **no longer a sync target.** Since harness 0.4.0 the file is the *project overlay* only (§1a extensions, §2 verifier roster, §5 not-observable); the framework half ships in the plugin as `references/qa-evidence-families.md` and is replaced on upgrade, not by sync. The overlay is seeded absent-only by `bootstrap.mjs seed` and is **never** rewritten afterwards — do not merge into it. **Roster completeness now holds at runtime instead of at sync time:** `qa-verify` routes any family with no §2 row to `NEEDS-HUMAN` naming the missing row. That guard is the only thing enforcing the rule this entry used to enforce; do not remove it believing sync still adds the row.
