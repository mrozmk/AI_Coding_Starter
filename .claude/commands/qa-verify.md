...
3b. **Guard unreachable tooling.** A built verifier is not a usable one. For each row, read the family's `Required tooling` class from `registry §2` and check this session's MCP roster for a server of that class:
   ...
   **`cross-device / viewport` needs the exact device server.** The class check is not enough (a desktop browser MCP satisfies it). For rows of this family: `qa-env.json → device_mcp_server` empty, **or** no server of that exact name in this session's MCP roster, **or** no live `qa-runtime-device` agent in the injected listing → `verdict: NEEDS-HUMAN`, `agent: (none)`, note naming which of the three is missing. Never route a touch-family row to the desktop server.
...
**What to pass:** `AC_SUBSET`, `SUBJECT`, `GATE_STATUS`, the router-resolved `BASE_URL`, and `MOUNT_TARGETS` built from `qa-env.json → component_mount_url_template`. **If that template is empty, skip the lane** and route its criteria to `NEEDS-HUMAN` with `no component mount point configured` as the reason. A project with no way to mount a component in isolation is a normal state — say so rather than pointing the verifier at a guessed route. Also pass **`KEY`** (the tracker key or spec slug) and **`ARTIFACTS_DIR`** (`qa-env.json → artifacts_dir`) — every browser verifier derives its per-AC artifact names from these two (`qa-runtime-ui.md → Artifacts`); never pass one concrete screenshot path for a subset with several `ac_id`s. **Sweep widths travel with the dispatch too:** the device lane gets **`SWEEP_WIDTHS`** = `qa-env.json → touch_sweep_widths`, and a `qa-runtime-ui` subset with tier-scoped criteria gets **`SWEEP_WIDTHS`** = `pointer_sweep_widths` — the verifiers never read `qa-env.json` themselves, so an unpassed list reads to them as "not configured". The device lane additionally gets **`BREAKPOINT_TOKEN_PREFIX`** = `qa-env.json → breakpoint_token_prefix` (empty = its token cross-check is skipped) — same rule: unpassed reads as unset.
