...
2. **Ignore config (auto-generate + review).** Ensure `.claude/map-codebase-ignore` exists. If absent, generate a **starter** from `.gitignore` (minus built-in defaults) plus detected excludable dirs (`tests/ __tests__/ fixtures/ docs/ examples/ scripts/ migrations/`), with all lines **commented out** and a header explaining `!`-negation, grouped under four commented section headers: **secrets** (active, never a candidate — no analyzer should even attempt the read), **generated artifacts** (codegen output, no design intent to extract), **platform projects & binary assets** (with `!` re-includes for the few config files worth a look, e.g. build flavors), **review at Checkpoint 1** (candidates such as `tools/`, docs generators, test dirs). The user reviews/uncomments it at Checkpoint 1. Apply built-in defaults + this file (negation supported) to filter the enumeration.
...
> This is a call-site contract. The script still tolerates a stringified payload (it parses **only**
> when `typeof args === "string"`, and a malformed blob throws) because that shape was observed in
> practice — the tolerance is a safety net, not permission to pass a string.
