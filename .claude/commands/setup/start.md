...
6. **Czy zespół używa Codex do drugiej opinii?** / **Does the team use Codex for a second opinion?**
   *Na tym komputerze: <zainstalowany | brak> — tylko informacja.* / *On this machine: <installed | absent> — informational only.*
   Tak · Nie / Yes · No
   *Codex to drugie AI, które sprawdza plany i kod. Pytam o zespół, bo ta odpowiedź trafia do wspólnego pliku. Jeśli nie: `groups.review` dostaje `false`, a skille pluginu zgłaszają rezygnację i pomijają Codex.* / *Codex is a second AI that checks plans and code. I ask about the team because this answer goes into a shared file. If no: `groups.review` is set to `false` and the plugin skills report the opt-out and skip Codex.* → `codex`, `commands.codex`
...
| `codex` | *(no starter files — cross-model review ships in the plugin since 0.3.0, gated by `groups.review`)* | `groups.review` in the project profile |
