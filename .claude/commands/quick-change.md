...
Assemble the prompt: orientation (*"read `.agents/reference/orientation.md` and follow its quick-mode steps; do not run it as a slash command"*) → *"read `$SCRATCH/quick-change-brief.md` — it is the full proposal; nothing has been implemented yet"* → the current files by path → then the **idea-mode task**, taken from [`/codex-review`](codex-review.md) Step 3 rather than reinvented:
