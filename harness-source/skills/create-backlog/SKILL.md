---
name: create-backlog
description: Generate .agents/backlog.md — the delivery map that operationalizes a PRD into epics, a task DAG and work packages the planning pipeline consumes.
argument-hint: "[path-to-prd]"
wrapper-description: Generate the delivery backlog from the PRD
---
...
|-------|----------|-------|---------|
| Product intent | `docs/PRD.md` → "Implementation Phases" | *what & why*, coarse phases, narrative | human, `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) |
| **Delivery map** | **`.agents/backlog.md` ← this command** | *in what dependency structure to ship it* — epics, task IDs, DAG, work packages, waves, DoD | human, `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) (per-WP scope) |
| Per-feature DAG | umbrella plan `## Execution Plan` | sub-steps of ONE work package | `/harness:orchestrate` (Claude Code) · `$orchestrate` (Codex) |

The PRD says *what & why* in prose. The backlog **operationalizes** the PRD's "Implementation Phases" into a dependency-structured inventory. It **does not** restate the PRD's narrative — it references it. The per-feature `## Execution Plan` (emitted later by `/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex)) is a finer DAG *inside* one work package; the backlog is the DAG *across* work packages.

> **The backlog is the input to `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex), not the output.** `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) designs ONE work package; the backlog decides *which* work packages exist, in what order, with what dependencies. Each work package row literally tells you the `/harness:brainstorm <topic> → spec → ` (Claude Code) · `$brainstorm <topic> → spec → ` (Codex)plan-feature` <spec>` invocation to run next.

<HARD-GATE>
This command produces a backlog — never code, never a spec, never a plan. Do NOT scaffold files, write implementation, or call `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex)/`/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex). It stops at `.agents/backlog.md`. The backlog is **opt-in**: a project that never runs this command has an untouched pipeline.
</HARD-GATE>
...
```
`create-prd` → `refresh-brief` → [`create-backlog`] → `brainstorm` (per work package) → `plan-feature` → `execute`
```
...
> **Loader Convention.** This command assumes `/harness:prime` (Claude Code) · `$prime` (Codex) already loaded `CLAUDE.md`, `index.md`, `project-brief.md`, `architecture.md`. Do **not** re-read those "for context". The PRD is this command's **source-of-truth for transformation** — read it directly (like `/harness:refresh-brief` (Claude Code) · `$refresh-brief` (Codex) does). If context isn't primed, ask the user to run `/harness:prime` (Claude Code) · `$prime` (Codex) first.
...
**Default to deciding, not asking** (same contract as `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex)). Derive epics, work packages, the DAG, and the MVP boundary from the PRD and state your assumptions. Reserve questions for the few genuine forks:
...
| Spec (from `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex)) | technical, detailed | the *how*: endpoints, token schema, edge cases |

The backlog **translates** the PRD's "what & why" into "in what structure to deliver it". So:

- ✅ A task is a **deliverable chunk of engineering work** — "Auth: JWT login + refresh", "Billing: Stripe webhook + idempotency", "Data layer: `.csv → DataFrame` adapter". `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) later takes the whole work package and designs the *how*.
...
- The right granularity test: *"Is this one coherent thing I'd hand to one `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) cycle?"* If it's three unrelated things → split into three tasks (maybe three work packages). If it's half a function → it's too small; fold it into its parent task.
...
- **`.agents/backlog.md` is the single source of truth for delivery order — Jira is an optional one-way mirror of it.** The backlog is created first, always, and holds the canonical "what to build, in what order" map. If a team uses Jira, its issues are derived *from* the backlog by exporting with `/jira bulk` (a manual/assisted export — there is no automatic sync), never maintained as a second, parallel list. The direction is one-way: backlog → Jira. Never reconstruct backlog ordering from Jira.
...
- **Work packages** group tasks into coherent themes. **One work package = one `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) → one spec → one `/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex).** A task ID does **not** map 1:1 to a command invocation — a work package does. Each WP row carries the exact pipeline entry to run.
...
- **Every work package must pass a readiness check (mini-INVEST) before it enters the pipeline.** Not the full six criteria — the three that actually save the AI flow:
  ...
  - **Small** — closeable in *one* `/harness:brainstorm → spec → /plan-feature` (Claude Code) · `$brainstorm → spec → /plan-feature` (Codex) cycle. If it's three unrelated things ("login + password reset + 2FA"), split into three WPs.
...
> `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) → one spec → one `/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex). Each WP carries a **readiness check** (is it ready to
...
|---------|-----------|-----------|--------------------|--------|
| **MVP-CORE** | E0-1, E0-3, … | — | `/harness:brainstorm <topic> → spec → ` (Claude Code) · `$brainstorm <topic> → spec → ` (Codex)plan-feature` <spec>` | TODO |
...
> `DONE` · `BLOCKED`. **Ref:** spec from `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) + plan from `/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex) — filled when the task
...
2. Start the first work package: `/harness:brainstorm <topic from the first MVP work package row>` (Claude Code) · `$brainstorm <topic from the first MVP work package row>` (Codex).
   Brainstorm writes a spec to `.agents/specs/`; `/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex) then writes a plan. **When a backlog
...
- **`Ref` + `Status` are written back automatically** by `/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex) (Status → `WIP`, Ref → spec) and `/harness:orchestrate` (Claude Code) · `$orchestrate` (Codex) (Status → `DONE`, Ref → spec + plan) — **but only if `.agents/backlog.md` exists.** No backlog → those steps are silent no-ops. This is the *only* automation; it is mechanical and safe.
- **Structural change (new epic, re-shaped DAG, re-scoped work package) is a deliberate, manual edit** — edit `.agents/backlog.md` by hand. Do not let an automated step rewrite the DAG; that is how a backlog starts "pretending to be the truth". (A dedicated guided-structural-edit command is a possible future addition — out of scope here.)
...
- This command does not call `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) or `/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex) — it only produces the map that feeds them.
