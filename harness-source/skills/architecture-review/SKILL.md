...
**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `execution` must be `true`; otherwise stop: `execution group disabled in the project profile`. On Codex this skill is `$architecture-review`; `/architecture-review` below names this skill on either host. On Codex this skill refuses: Phases 1 and 3 fan out read-only subagents over the codebase, and Codex has no agents — a single-threaded sweep would be a different, weaker product.

**Input:** the module or area to focus on; empty means the whole codebase. A bare `--codex` token adds the second independent sweep of Phase 0.

Surface architectural friction across the **whole codebase** (or a named area) and propose **deepening opportunities** — refactors that turn shallow modules into deep ones. The aim is leverage, locality, testability, and AI-navigability. This review **reports and reasons; it never edits code.** The actual refactor, once a candidate is chosen, goes through `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) → `/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex) → `/harness:execute` (Claude Code) · `$execute` (Codex).

Resource files for this skill ship with the plugin as `references/architecture-review/GLOSSARY.md`, `references/architecture-review/DEEPENING.md`, `references/architecture-review/INTERFACE-DESIGN.md` and `references/architecture-review/report-template.html` — read them from the installed plugin root.

**Vocabulary is load-bearing.** Use the terms in `skills/architecture-review/GLOSSARY.md` **exactly** in every observation and card — module, interface, depth/deep/shallow, seam, adapter, leverage, locality. Don't drift into "component," "service," "API," or "boundary." Consistent language is the point.
...
Strip a bare `--codex` token from the input before treating the rest as the focus argument. **Without the flag, skip this phase entirely** and run the review exactly as it ran before — a full `high`-effort Codex sweep of a whole codebase is slow and expensive, and this command is already on-demand.
...
**Why the same method, not a free-form second opinion.** The method *is* this command's product. A reviewer without `GLOSSARY.md` and `DEEPENING.md` does not return deepening candidates — it returns a generic list of code smells, which is `/harness:deep-review`'s lens and is explicitly out of scope here (see Phase 2b). So Codex gets the full method. The independence we want lives elsewhere:
...
**Precondition.** The sweep is an opinion, so it needs both `execution` and `review` enabled in the project profile. With `review` off, say the project opted out and continue with one sweep. With the groups on and the `codex` CLI absent this is **blocked, not skipped** — report the block and continue with one sweep. **Never fabricate a Codex sweep** (project rule: never fake it).

**Assemble the prompt** in this order:

1. **Orientation** — pack `references/orientation.md` into the prompt file itself, under the line "First, orient yourself in this project by following these quick-mode steps." The child reads the prompt, never a path in the project.
2. **The method** — pack `references/architecture-review/GLOSSARY.md` and `references/architecture-review/DEEPENING.md` into the prompt as well. "They define the vocabulary and the method: depth as leverage per unit of interface complexity, the deletion test, seam discipline, the four dependency categories. Use those terms exactly."
...
6. **Output** — "Return the whole sweep as your final message." The orchestrator owns liveness (`<run>/status.json`), so the prompt needs no heartbeat convention.
...
**Launch** through the supervised orchestrator — read mode, prose (no `--schema`). The caller contract is `references/executor-contract.md`; read it before the first call.

Write the assembled prompt to a file in the session scratchpad, then:
...
node <plugin_root>/scripts/executor-orchestrator.mjs \
  ...
  --prompt-file <scratch>/architecture-sweep.prompt.md --scratch <scratch>
```

Run it once with `--dry-run yes` first: that writes `<run>/manifest.json` — the exact argv, sandbox and ceiling — and spawns nothing. Then run it for real with `run_in_background: true` and **go straight into Phase 1** rather than waiting; that concurrency is the point of running it first. Never inline `codex exec`, and never ask for `--mode write` — a review mutates nothing, and the orchestrator rejects a read-mode child that touched the tree.

**The result file is the signal, never process exit.** The orchestrator writes `<scratch>/exec-<run_id>.json`; `<run>/status.json` is the heartbeat to poll (first check after ~8 min, then every 5 — `ScheduleWakeup`, cancelled on every exit path). Print `summary_line` verbatim in the report. Dispositions, per `references/executor-contract.md`:

- `status: completed` with `baseline.status: ok` → read `output_file` and merge the sweep.
- `execution: not-executed` → one technical retry (`--technical-retry yes`); still not executed → render from Claude's sweep alone and say so, with `summary_line` in the report.
- timeout, empty output, or a baseline deviation → render from one sweep and say so. A deviation is never merged: a read-only child that wrote is a rejected opinion, not a finding.
- `status: skipped` because `groups.review` is off → say the project opted out of cross-model review; that is not a failed sweep.
...
1. `.agents/memory/architecture.md` — our module map and naming rules. Use its domain vocabulary for *what the modules are*; use `skills/architecture-review/GLOSSARY.md` for *how they're shaped* (depth/seam/adapter).
...
> **Guardrail — do not audit content quality here, in any phase.** Naming, comment noise, duplication, hacky constructs, giant functions, spaghetti conditions and type sloppiness are `/harness:deep-review`'s standards, and this command neither applies them nor reports them. A hotspot row says *"this file is large, branchy and untested"* — a measurement. It never says whether the code inside is good. Blurring that line reinstates the merge these two commands were split to avoid, and buries the deepening candidates under nits.

For candidates that involve **merging a cluster of shallow modules**, classify dependencies before recommending the merge — the four categories (in-process / local-substitutable / remote-owned / true-external) each set a different testing strategy. See `skills/architecture-review/DEEPENING.md`.
...
Render the candidates into `skills/architecture-review/report-template.html`, replacing the `{{placeholders}}`, and **save the rendered file to** `.agents/reference/architecture-review-YYYY-MM-DD.html` (use today's date from context). **Never write to `/tmp`, and never into the source tree** — the report is a reference artifact.
...
The candidate cards above cover **depth**. They deliberately say nothing about the other half of "this codebase is a mess" — file size, spaghetti conditions, duplication, naming, comment noise. That half belongs to `/harness:deep-review`, which is per-file and can apply fixes. This section is the **hand-off between the two**: it names *where* to point that command, using only the signals collected in Phase 1.
...
Hotspots are **not ranked against candidates and never merge into them** — they are a different question with a different owner. A file may legitimately appear in both, and when it does, say so in the row: the deepening comes first, since it may delete the code a `/harness:deep-review` pass would otherwise have cleaned.
...
- **One line on hotspots** — how many were listed and the single worst one, with its `/harness:deep-review` command inline so it can be run without opening the report. Omit the line when no hotspot crossed the thresholds, and say that instead.
...
Once the user picks a candidate, design its interface deliberately using **Design It Twice**: see `skills/architecture-review/INTERFACE-DESIGN.md`. Frame the problem space, generate 2–3 radically different interface variants (in parallel via the Agent tool, the way `/harness:design` (Claude Code) · `$design` (Codex) fans out variants), then compare them by depth / locality / seam-placement and give an **opinionated** recommendation (or a hybrid).

Walk the design tree with the user — constraints, dependency category (per `skills/architecture-review/DEEPENING.md`), the shape of the deepened module, what sits behind the seam, which tests survive.

**Side effects only happen with the user's explicit consent:**

- **Naming a deepened module after a concept that isn't in our map?** Offer to add or sharpen the term in `.agents/memory/architecture.md` so future reviews speak the same language. Ask first.
...
**Never rewrite code here.** This phase produces a chosen, interface-designed deepening — the actual implementation is handed off to `/harness:brainstorm` → `/harness:plan-feature` → `/harness:execute`.
