...
**Input:** an optional subcommand and its arguments. Triggered by: a Confluence Cloud wiki URL (regex `https?://[^/]+/wiki/`, e.g. https://<workspace>.atlassian.net/wiki/spaces/<SPACE>/overview), the words "confluence", "wiki", "article", "artykuł", "space", "read this page", or authoring intents — "create/write an article", "new page", "publish", "stwórz artykuł", "nowy artykuł". Also via explicit /confluence invocation. First argument may be a wiki URL, a numeric page id, raw CQL, free-text search terms, `new "Title"`, or `publish <draft-file>`. For Jira issues use /jira — this skill is Confluence only.

**Tool surface:** every MCP call goes through the Atlassian server: `mcp__atlassian__confluence_search`, `mcp__atlassian__confluence_get_page`, `mcp__atlassian__confluence_get_page_children`, `mcp__atlassian__confluence_get_comments`, `mcp__atlassian__confluence_get_labels`, `mcp__atlassian__confluence_create_page`, `mcp__atlassian__confluence_update_page`, `Write`, `Read`, `Edit`, `Bash`. The server, its permission tiers and the `JIRA_*` / `CONFLUENCE_*` environment variables are **project-side**, never shipped by the harness.
...
`node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `groups.confluence` must be `true`; otherwise stop: `confluence group disabled in the project profile`. A `blocked` result (profile missing, invalid or conflicting) stops here with the reason.
