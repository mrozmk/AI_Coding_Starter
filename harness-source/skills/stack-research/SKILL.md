---
name: stack-research
description: Technical project analysis at the PRD stage — technology selection, feasibility assessment and the risks each option carries, with the decision left to the user.
argument-hint: "[path-to-prd | topic]"
wrapper-description: Technology selection and feasibility assessment
---

# `stack-research` — Technology Discovery & Feasibility

**Input:** an optional path to the PRD or a topic sentence. Empty → `docs/PRD.md`.
...
`/harness:stack-research` (Claude Code) · `$stack-research` (Codex) is a **technology selection tool**, not an implementation tool. Use it when you need to answer: *"What should we build this with, and why?"* — before writing a single line of code.

Two modes, detected from the input:

| Mode | Trigger | Source of truth | Output target |
|------|---------|-----------------|---------------|
| **Project-level** | input empty | `docs/PRD.md` (full product context) | Updates PRD `Technology Stack` / `Core Architecture & Patterns` sections |
| **Feature-level** | input names an area (e.g. `realtime updates`, `auth`, `search`) | PRD + the named area | Brief saved as reference; PRD update optional |
...
For project-level mode (no input topic), use `<kebab-case-topic>` = `project-stack` or a 1-2 word descriptor of the product (e.g. `audit-ai-stack`, `marketplace-stack`).
...
2. **Append a one-line entry to `.agents/memory/decisions.md`** (newest at END per starter convention):
   ...
   **Brief:** `.agents/specs/YYYY-MM-DD-stack-research-<topic>.md`
   ```
...
## What `/harness:stack-research` (Claude Code) · `$stack-research` (Codex) is NOT

| This is not… | Use instead |
|--------------|-------------|
| An implementation plan | `/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex) |
| A feature specification | `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) |
| A codebase or architecture analysis | `/harness:analysis` (Claude Code) · `$analysis` (Codex) |
| A quick opinion on tools | Normal conversation |
| A PRD generator | `/harness:create-prd` (Claude Code) · `$create-prd` (Codex) |
| A brief regenerator | `/harness:refresh-brief` (Claude Code) · `$refresh-brief` (Codex) (call AFTER `/harness:stack-research` (Claude Code) · `$stack-research` (Codex) updates PRD) |
