---
name: gates-check-quality
description: Run the project's declared quality gates — format, lint, type-check and the code-structure size limits — and report findings. Reads the commands from CLAUDE.md; assumes no toolchain and fixes nothing.
---

# /harness:gates-check-quality — Run project quality checks

**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `execution` must be `true`; otherwise stop: `execution group disabled in the project profile`. On Codex this skill is `$gates-check-quality`; `/harness:gates-check-quality` (Claude Code) · `$gates-check-quality` (Codex) below names this skill.
...
Read `CLAUDE.md` section `Validation` first (Claude Code injects it every session; on Codex `AGENTS.md` points at it) — it is the declared source of truth for quality gates (the same one `/harness:gates-verify-implementation` (Claude Code) · `$gates-verify-implementation` (Codex) and `/orchestrate` read) — then fall back to `Commands`, plus `Code Structure & Modularity` for the size limits. Extract:
