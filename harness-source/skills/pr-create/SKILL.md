---
name: pr-create
description: Prepare a pull request — push the branch, derive title and destination from the Branch model and the tracker, fill the repo PR template honestly, print the create-PR URL. Prepares only, never opens or merges.
...
wrapper-description: Prepare a pull request (never opens or merges)
---

# /pr-create — prepare a pull request (host-agnostic)

**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `git` must be `true`; otherwise stop: `git group disabled in the project profile`. On Codex this skill is `$pr-create`; `/pr-create` below names this skill on either host.
...
**Input:** an optional issue key (`[A-Z]+-\d+`); when absent it is derived from the branch name.

---

## Guardrails

- **Never create the PR, never merge, never commit.** Dirty tree → tell the user to run `/harness:commit` (Claude Code) · `$commit` (Codex)
...
- **The only outward action is the push**, delegated to `/harness:push` (Claude Code) · `$push` (Codex) (own branch, never `--force*`).
...
- Dirty tree with un-committed owned changes → STOP: run `/harness:commit` (Claude Code) · `$commit` (Codex) first. Unrelated untracked
...
- `KEY` = the input key if given, else the first `[A-Z]+-\d+` match in `BRANCH`, uppercased. May
...
Invoke `/harness:push` (Claude Code) · `$push` (Codex). It sets the upstream on first push, runs the secret scan, and asks before a
