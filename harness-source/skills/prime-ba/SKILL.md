---
name: prime-ba
description: Load product context for the Business Analyst role — PRD, brief, business model and backlog — so requirement work starts from what the product promises rather than from the code. Read-only.
argument-hint: "[deep]"
wrapper-description: Load product context for the Business Analyst role
---
...
List `.agents/specs/` — every file is an approved design doc from `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex). Each spec represents a designed-but-not-necessarily-implemented feature.
...
If no PRD exists, say so and recommend running `/harness:create-prd` (Claude Code) · `$create-prd` (Codex) first.
...
- PRD features with **no spec** in `.agents/specs/` — candidates for `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex)
- Specs with **no matching Jira epic** — candidates for creating Epics in Jira
- Raw sources in `.agents/sources/` that have **no derived spec** — candidates for PRD refresh or `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex)
...
- `/harness:brainstorm <feature X>` (Claude Code) · `$brainstorm <feature X>` (Codex) — spec gap identified in PRD
- `/jira create epic` — spec exists but no Jira epic
- `/harness:create-prd` (Claude Code) · `$create-prd` (Codex) — no PRD yet, product direction unclear
...
- Focus on **what** and **why**, not **how** (leave implementation detail to `/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex) and developers)
