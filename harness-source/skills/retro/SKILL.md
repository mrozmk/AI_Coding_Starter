---
name: retro
description: Generate an evidence-based session retrospective from the host transcript — paths, counts, tool-call references, timestamps. Refuses to write an opinion-only or self-flattering report; the cheap retro is the dangerous one.
argument-hint: "[--force] [--transcript <path>]"
wrapper-description: Evidence-based session retrospective from the transcript
---

# retro — evidence-based session retrospective

**Input:** optional `--force` (override a failed quality gate, loudly) and `--transcript <path>` (analyse a past session).

**Prerequisite:** none — this is an ungrouped session tool, like `prime` and `handoff`.

`retro` runs at the end of a long or frictional session and produces a structured markdown report grounded in **hard evidence**: paths, counts, tool call references, transcript timestamps. It does not produce opinions, vibes, or self-assessments.

**This command is project-agnostic.** It works in any codebase regardless of stack (Python, JS, monorepo, single script, etc.).

**This command refuses noise.** If the session was trivial, friction-free, or if the model can't produce evidence-backed claims, this skill REFUSES to write a retro. The cheap retro is the dangerous one — it pollutes the historical record with false reassurance.
...
**The transcript source is host-dependent, and only Claude Code has a documented one.** The layout below (`~/.claude/projects/<slugged cwd>/<session id>.jsonl`, keyed off `CLAUDE_CODE_SESSION_ID`) is Claude Code's. **On Codex there is no documented transcript path**, so this skill has no evidence source of its own there: it must **refuse and say why**, unless the user passes `--transcript <path>` pointing at a `.jsonl` in the same line-delimited shape. Degrade explicitly with

```
no host transcript source on this host — pass --transcript <path>, or run retro on Claude Code
```

and stop. Never substitute conversation memory for the transcript: hard rule 1 exists precisely because a remembered session is the thing this skill refuses to report on.

Run this Bash one-liner on Claude Code. If anything fails, STOP and tell the user.
...
- `SID` is empty → `CLAUDE_CODE_SESSION_ID env var not set. Cannot identify current session. Run this skill from inside a live host session.`
- Transcript file missing → `Transcript not found at <path>. If you're running this from a different cwd than the original session, pass --transcript <path>.`
...
Outputs a single JSON object to stdout with all metrics the retro needs.
...
Options:
  1. Run this skill from inside a project with .agents/ or .claude/ structure.
