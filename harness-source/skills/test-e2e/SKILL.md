---
name: test-e2e
description: Write E2E tests for a user flow using MCP Playwright browser automation — explore the running UI, produce a test plan, get it approved, then generate the specs.
argument-hint: "[flow-name | jira-key (e.g. CS-1) | empty for plan-driven]"
---

# Test E2E: Explore → Plan → Generate

**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `execution` must be `true`; otherwise stop: `execution group disabled in the project profile`. On Codex this skill is `$test-e2e`; `/test-e2e` below names this skill on either host.

**Input:** a flow name, a Jira issue key, or nothing. Empty (or `all`) means plan-driven: the flow list comes from the latest plan's Testing Strategy. Step 0.1 detects which.
...
### 0.1 Detect input mode from the input
...
**Soft-fail for jira-driven mode:** if the input looks like a Jira key but MCP atlassian is not configured (any of `JIRA_URL` / `JIRA_USERNAME` / `JIRA_API_TOKEN` missing from `.env`):

> ⚠️ "The input looks like a Jira key, but MCP atlassian is not configured. Treat it as a flow name and continue, or stop and configure Jira first?"
...
### 0.3 Project context — delegated to `/harness:prime` (Claude Code) · `$prime` (Codex)

**This command does NOT re-load project context.** UI language, module map, domain rules, and high-level goal are project-wide context that `/harness:prime` loads once per session. Re-reading them here would duplicate work and burn tokens (especially `docs/PRD.md` — can be 2000+ lines).
...
**If those aren't in your conversation context yet** (no earlier `/harness:prime` in this session, or they were absent), say so and ask the user to run `/harness:prime` first, then re-invoke this command. Do not silently fall back to reading the PRD.

See `.agents/memory/index.md` for the project-wide rule.
...
### Ready for commit
Confirm all tests pass before handing off to `/harness:commit` (Claude Code) · `$commit` (Codex).
