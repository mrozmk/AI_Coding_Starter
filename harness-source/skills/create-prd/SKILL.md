---
name: create-prd
description: Create a Product Requirements Document from the conversation — executive summary, mission, users, MVP scope, functional and non-functional requirements, success metrics. Writes docs/PRD.md; never writes code.
argument-hint: "[output-filename]"
wrapper-description: Create a Product Requirements Document from conversation
---

# Create PRD: Generate Product Requirements Document

**Input:** an optional output path for the PRD. Empty → `docs/PRD.md`.
...
Write the PRD to the path given as input (default: `docs/PRD.md`).

**Before writing, ensure the target directory exists.** If `docs/` (or the parent directory of the resolved output path) is missing, create it first — `mkdir -p <parent-dir>`. This guarantees the PRD lands in a predictable location even on a fresh starter clone where `docs/` has not yet been initialized.
