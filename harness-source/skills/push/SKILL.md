---
name: push
description: Push local commits to origin on the current branch — detects the branch, confirms a protected-branch push, sets the upstream on first push, pushes new tags, never forces.
wrapper-description: Push to origin
---

# /push — push to origin

**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `git` must be `true`; otherwise stop: `git group disabled in the project profile`. On Codex this skill is `$push`; `/push` below names this skill on either host.
...
- If push is rejected (non-fast-forward):
  ...
  - Inform the user: "Push rejected — remote has commits not in local history. Run `/harness:pull` (Claude Code) · `$pull` (Codex) first, then push again."
  - Stop here.
