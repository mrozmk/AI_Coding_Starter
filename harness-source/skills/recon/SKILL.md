---
name: recon
description: Assemble broad context BEFORE brainstorm — fan out research over memory, prior specs/tests, the integration surface, external docs, the open web, and (for technical features) similar GitHub repos, then synthesize an IN-SESSION cited recon brief. Writes no files.
wrapper-description: Assemble broad context before you design
...
**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `execution` must be `true`; otherwise stop: `execution group disabled in the project profile`. On Codex this skill is `$recon`; `/recon` below names this skill on either host.

**Input:** the feature idea, plus the optional switches `--web`, `--github`, `--local`. Empty → the topic is resolved in Step 0 (default: the next free backlog task), exactly like `brainstorm`.

## Topic

The topic is the input; if it is empty, Step 0 resolves it (default: the next free backlog task), exactly like `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex).

<HARD-GATE>
Recon is **read-only** and produces its brief **in the session** — it presents a synthesized, cited context dossier in the conversation and **writes no files** (no `.agents/…`, no spec, no code). Its whole job is to answer *"what already exists / was tried / is decided / is unknown about this — here and out in the world?"* so that `/harness:brainstorm` starts from evidence instead of a blank slate. It does **not** design, choose an approach, or write a spec — those belong to `/harness:brainstorm`. The only thing it may *write* is nothing; if the brief is worth keeping, it **proposes** the user run `/harness:handoff` (Claude Code) · `$handoff` (Codex) — it never persists on its own.
</HARD-GATE>

<WHEN-TO-RUN>
Run recon when the idea **leans on prior work** ("we tested a lot of variants", "convert X into Y", "reuse what we built for Z") or **crosses into unfamiliar tech / a domain others have already solved on GitHub**. That is where `/harness:brainstorm`'s light Step-1 context touch is too shallow. **Skip recon** for a small, self-contained change with no meaningful prior art — go straight to `/harness:brainstorm`. Recon is the heavy, parallel dig; brainstorm is the design.
</WHEN-TO-RUN>

> **Division of labor.** `/recon` **gathers** (deep, parallel, read-only, in-session). `/harness:brainstorm` **designs** (approaches → spec). Because the brief lives in the conversation, **a `/harness:brainstorm` run in the same session already has this context** — no file, no re-derivation, no wiring. Run recon, read the brief, then run ``/harness:brainstorm` <topic>` in the same session.
...
Same resolution order as `/harness:brainstorm` Step 0:

1. **Explicit topic in the input** → use verbatim.
...
> **Loader Convention.** Assume `/harness:prime` (Claude Code) · `$prime` (Codex) already loaded `CLAUDE.md`, `index.md`, `project-brief.md`, `architecture.md`. Do **not** re-read them — the research agents read what they need. If context isn't primed, ask the user to run `/harness:prime` first.

From the raw idea, write down (briefly):

- **Goal** — one line: what the user wants to end up with.
- **Why** — the problem behind the request. If genuinely unclear, this is the one thing you may ask up front (mirrors `/harness:brainstorm`'s WHY-GATE).
...
**Switches** (mirror `/harness:brainstorm`'s `External docs required` flag):
...
On Claude Code spawn the internal agents in a **single message with multiple `Task` calls**; on Codex, which has no agents, run the same waves sequentially in this session under the same output contract. The main thread does **no** research itself — it briefs the agents and later synthesizes. Give every agent the goal, the why, the named handles, and this **output contract**:
...
## Gaps & open questions for `/harness:brainstorm`
...
**Gaps & open questions** and **Suggested framing** are the payload for `/harness:brainstorm` — spend the most care there.
...
2. Tell the user: *"Recon done — this brief is now in-session context. Run ``/harness:brainstorm` <topic>` in this session and it starts from here."*
3. **Offer, don't act:** *"Want this preserved beyond the session? Run `/harness:handoff` and I'll compact it into a doc."* Do not write any file yourself.

---

## Key Principles

- **Read-only, in-session** — recon writes nothing; the brief lives in the conversation. Persisting is the user's call via `/harness:handoff`.
