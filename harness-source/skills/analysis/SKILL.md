---
name: analysis
description: Deep analysis of a problem, decision, or codebase area — no code written, no files created. Use when a surface-level answer would be risky: architectural trade-offs, root-cause investigations, "should we do X or Y".
---

# `/harness:analysis` (Claude Code) · `$analysis` (Codex) — deep analytical pass

**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `execution` must be `true`; otherwise stop: `execution group disabled in the project profile`. On Codex this skill is `$analysis`; `/analysis` below names this skill on either host.

`/harness:analysis` is a **thinking tool**, not a doing tool. Use it when the answer matters enough that a surface-level response would be risky: architectural trade-offs, root-cause investigations, "should we do X or Y", non-trivial decisions before they harden into code.

The fact that the user invoked `/harness:analysis` is itself a signal — **the topic carries weight**. A shallow response is a failed `/harness:analysis`.

---

## Hard rules (never violate)

1. **Never write or edit code during `/harness:analysis`.** No `Edit`, `Write`, `MultiEdit`, `NotebookEdit`, no "tiny fix along the way", no refactor, no staged patches. If implementation is warranted, surface it as a next step for the user to run explicitly (e.g. `/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex), `/harness:execute` (Claude Code) · `$execute` (Codex)) — do not start it yourself.
...
2. **Check the project knowledge layers:**
   - `.agents/memory/` — `decisions.md`, `patterns.md`, `errors.md`, `api.md` (and domain files) may already resolve the question.
   - `.agents/specs/` and `.agents/plans/` — an active or past feature may have already analyzed this.
   - `.agents/reference/` — stable domain / API references.
...
5. **Scope-box your investigation.** If the set of files you need to read keeps growing past roughly 15–20, or the investigation branches into multiple unrelated subsystems — **stop, surface the current findings, and check in with the user** before continuing. Rabbit-holing is a failure mode of `/harness:analysis`: a partial answer with a clear stop signal is more useful than an endless dive. When you stop, explain *what* you would investigate next and *why*, so the user can decide whether to greenlight it.
...
## What `/harness:analysis` is NOT

| This is not… | Use instead |
|--------------|-------------|
| An implementation plan | `/harness:plan-feature` |
| A feature specification | `/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) |
| A PR review | `/review` |
| A "quick tweak" opportunity | A normal conversation — not `/harness:analysis` |
...
| **Next step** | `/harness:brainstorm` to turn this into a design doc |
```
...
This block is the single most valuable part of `/harness:analysis` — it gives the user a handle to say "go back and check X" instead of silently trusting an incomplete view. **Never skip it**, even when the table looks tidy.
