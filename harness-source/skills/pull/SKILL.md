---
name: pull
description: Pull the latest changes on the current branch from origin — detects the branch, auto-stashes and restores local changes, stops on conflicts with the conflicting files listed.
wrapper-description: Pull latest code from origin
---

# /pull — pull the latest code from origin

**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `git` must be `true`; otherwise stop: `git group disabled in the project profile`. On Codex this skill is `$pull`; `/pull` below names this skill on either host.
...
- Run `git log --oneline HEAD..origin/<branch>` to see incoming commits
  - If `origin/<branch>` does not exist on remote (new local branch), inform the user: "`origin/<branch>` does not exist — nothing to pull. Push first with `/harness:push` (Claude Code) · `$push` (Codex)."
