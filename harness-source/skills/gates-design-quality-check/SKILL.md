---
name: gates-design-quality-check
description: Pixel-perfect audit of an implemented UI section against its reference design — visual, structural, semantic, behavioural, responsive and accessibility deviations. Report-only.
argument-hint: "[section-name] [reference-file?]"
---

# /harness:gates-design-quality-check — Design-Parity Audit

**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `execution` must be `true`; otherwise stop: `execution group disabled in the project profile`. On Codex this skill is `$gates-design-quality-check`; `/harness:gates-design-quality-check` (Claude Code) · `$gates-design-quality-check` (Codex) below names this skill.

**Input:** the section or component to audit (required) and, optionally, the reference — a Figma node URL/ID or a path to a static artifact. §1 resolves both.
...
This command is the inverse of `/harness:gates-verify-implementation` (Claude Code) · `$gates-verify-implementation` (Codex): that one checks code quality against a plan; this one checks **design fidelity** after any implementation pass (initial build, fix-up, redesign).
...
>
> This wording is deliberately identical to `/harness:gates-verify-implementation` → §5 Design Compliance. Two gates stating the same rule differently is how one of them stops being believed — change both or neither.
...
- **26+** → BLOCK. Do **not** dump all findings into chat. Instead:
  ...
  3. Recommend `/harness:execute` (Claude Code) · `$execute` (Codex) (or `/orchestrate`) against the new plan once the user approves.

The threshold exists because dumping 50+ findings into chat buries the conclusion and burns context. The plan file is the artifact the implementer (and `/harness:execute`) actually consumes.

---

## 9. Output template

> **`VERDICT:` line — the machine-readable token inline callers and `harness:orchestrator-designer` parse.** `passed` = no findings; `failed` = any finding; `not-verified` = UI in scope but parity could not be checked (reference tool not connected or errored / empty payload, no resolvable node or artifact, app unreachable). Emit `not-verified` with the reason instead of inventing values or falling back to a weaker source — it is **not checked, never a pass**.
...
- [If WARN/BLOCK: implement fixes (or run `/harness:execute` on the linked plan), then re-run `/harness:gates-design-quality-check` (Claude Code) · `$gates-design-quality-check` (Codex)]
```
