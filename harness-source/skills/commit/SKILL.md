---
name: commit
description: Create a clean git commit scoped to the files this session edited — foreign changes from parallel sessions are reported, never swept in; conventional-commit message, memory checkpoint, no push.
wrapper-description: Create a git commit
---

# /commit — create a git commit

**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `git` must be `true`; otherwise stop: `git group disabled in the project profile`. On Codex this skill is `$commit`; `/commit` below names this skill on either host.
...
3. **Determine "your" files** — the set you actually touched in this session:
   - Reconstruct from your transcript: every successful `Edit`, `Write`, `NotebookEdit`, `MultiEdit`, plus any `mv`/`rm`/file-creating `Bash` you ran. On Codex the edit trail is every `apply_patch` call plus shell writes (`cat >`, `sed -i`, `mv`) in this session. Track absolute paths; normalize to repo-relative.
...
7. Stage the approved set with explicit `git add <path>` calls, one path per file, **in a separate Bash call from the commit** — the `guard-commit` hook (plugin `hooks/hooks.json`; legacy `guard-commit.sh`) inspects the staged set before a chained `git add … && git commit` runs and blocks it as an empty commit. Do NOT use `git add .`, `git add -A`, or `git add -u` — all three bypass the scoping you just established. `-u` is the easy one to miss: it stages every *tracked* modification, which is exactly the FOREIGN set you just classified out. Skip `.env`, credentials, large binaries, `node_modules` even if they appear in your set (refuse + warn the user).

8. Create the commit with the generated message — **no further confirmation needed at this point**.
9. **Memory checkpoint** — read the project's `.agents/memory/reflection-protocol.md` first: its save-or-not bar and de-dup guard govern this step (default outcome: save nothing; if `/execute` or `/check-implementation` already reflected in this flow, there is usually nothing left). Then review the work done in this commit and ask yourself:
