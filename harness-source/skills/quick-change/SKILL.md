---
name: quick-change
description: The fast lane for small changes — a short in-session plan (no spec, no plan file), a mandatory independent Codex review of that plan BEFORE any code is written, then implement, code-review (low effort), deep-review, and the project's Validation gates. The review cannot be opted out of. For work where brainstorm → plan-feature → execute is too heavy, but "just do it" is too risky.
wrapper-description: The fast lane for small changes
argument-hint: "<what to change>"
---

# /quick-change — Fast Lane for Small Changes

**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `execution` must be `true`; otherwise stop: `execution group disabled in the project profile`. On Codex this skill is `$quick-change`; `/quick-change` below names this skill on either host. On Codex this skill refuses: Phase 4 leans on the host-provided `code-review` and the lane's judge-is-not-the-fixer split, neither of which Codex can honour.

**Input:** what to change, in the user's own words. Empty → ask; this lane never guesses its own scope.

The full pipeline (`/harness:brainstorm` (Claude Code) · `$brainstorm` (Codex) → `/harness:plan-feature` (Claude Code) · `$plan-feature` (Codex) → `/harness:execute` (Claude Code) · `$execute` (Codex) → `/check-implementation`) is right for a feature and absurd for a two-file change. But the alternative people fall back on — "just do it" — is where the not-quite-good-enough quick fixes come from: no plan, no second opinion, no cleanup, no gate.

This command is the middle. **It costs one short plan and one review, and it writes nothing to the repo's knowledge layer.**

```
short plan (in chat) → Codex reviews the PLAN → implement → /code-review (low) → `/harness:deep-review` (Claude Code) · `$deep-review` (Codex) → Validation gates
```

The load-bearing move is **where the review sits**: before the code, not after. A wrong direction caught while the plan is three bullet points costs a rewrite of three bullet points. Caught after implementation, the cost is already sunk and the pull is to patch it rather than redo it. That is the executor's **read mode** used as an idea review, with the ceremony of a full plan removed.

> **Loader Convention.** Assumes `/harness:prime` (Claude Code) · `$prime` (Codex) already ran. Do not re-read `CLAUDE.md`, `architecture.md` or the brief here.

---

## What this is NOT

- **`/harness:execute`** — runs a written plan file from `.agents/plans/active/`. `/quick-change` has no plan file, by design; the plan lives in the conversation and dies with it.
...
- **`/harness:plan-feature`** — produces a durable, grilled plan artifact. `/quick-change` produces a disposable one.
...
Sugeruję: <"`/harness:brainstorm` → `/harness:plan-feature`" dla niejasnego zakresu | "`/harness:plan-feature`" gdy kierunek jest jasny | "/check-implementation" jeśli kod już powstał>.
...
**Snapshot the working tree here too:** `git status --porcelain`. Whatever is already dirty is **not yours** — Phases 3.5–6 act only on files *this run* touched. Both `/code-review` and `/harness:deep-review` mutate code, so a review scoped to "the changed files" on a tree that was dirty at the start would rewrite someone else's uncommitted work and report it as this run's. If the pre-existing changes overlap the files you are about to touch, say so and ask before proceeding — that is the one case where the two cannot be separated.
...
**Exactly two things can leave a run without the opinion, and neither is a choice:** the project profile has `review` off (an explicit opt-out, reported as such), or the orchestrator failed to produce a result twice in a row (below). With `review` on and the `codex` CLI absent, the phase is **blocked, not skipped** — stop and report it. In both cases the run continues, and the report states the absence on its own line. Those are tool failures, not decisions — never present them as one, and **never fabricate a review** (project rule: never fake it).

**Write the brief to the scratchpad, not to `.agents/`.** Codex cannot see this conversation, so the brief must stand alone — the plan from Phase 1 verbatim, plus the paths of the files it would change and the current behaviour it changes. 10–20 lines into `$SCRATCH/quick-change-brief.md`. (This is the sanctioned shortcut for changes where `/harness:handoff` (Claude Code) · `$handoff` (Codex) would be overkill.)

Assemble the prompt: orientation (pack `references/orientation.md` into the prompt file — the child reads the prompt, not a path in the project) → *"read `$SCRATCH/quick-change-brief.md` — it is the full proposal; nothing has been implemented yet"* → the current files by path → then the **idea-mode task**:
...
Append the anti-forcing clause verbatim: *"Calibrate to what is actually there. A clean result is a valid, valuable outcome — do not manufacture findings, pad the list, or inflate a nit into an objection."* No heartbeat instruction is needed; the orchestrator owns liveness. **Seed nothing else** — no hypotheses, no "I think X is fine", no expected findings. Independence is the product.

Spawn through the supervised orchestrator — read mode, prose (no `--schema`):

```bash
node <plugin_root>/scripts/executor-orchestrator.mjs \
  --project-root <project_root> --plugin-root <plugin_root> --author-host claude \
  --mode read --effort high --timeout-minutes 25 \
  --prompt-file <scratch>/quick-change.prompt.md --scratch <scratch>
```

**Lifecycle — do not invent one here.** Launch with `run_in_background: true` (never a trailing `&`) and follow `references/executor-contract.md`, which carries this call site's 25-minute ceiling and the polling cadence; `<run>/status.json` is the heartbeat, and the cancel-on-every-exit rule (`ScheduleWakeup stop: true`) applies. The ceiling is short by design — a plan review for a small change that runs past it is hung, not thorough — and hitting it fails **open**: continue without the opinion and report it, never stall the lane. Effort is pinned `high` (the fast lane is fast because of its ceiling, not a weaker model).

**Wait for it. Do not start implementing while it runs** — the entire value of this phase is that no code exists yet, and a half-written implementation turns an open question into something you defend. Poll `<run>/status.json` for liveness; never read the run logs in full.

Done means `status: completed` in `<scratch>/exec-<run_id>.json`, never process exit alone; print `summary_line` verbatim in the report. `execution: not-executed`, an empty final message or a malformed result is a **failure, not a clean review** — one technical retry (`--technical-retry yes`), then continue without the opinion and say so plainly. Do not ask the user for permission to skip; a question at that moment is the removed flag wearing a different hat, and it puts the decision on whoever is least able to judge what the missing review would have caught.

> The orchestrator writes a fresh run directory per call, so a result file is always *this* run's review — a stale artifact cannot satisfy the mandatory phase. Do not reintroduce a done-check that reads a fixed output path.
...
Bugs before structure — the same ordering `/check-implementation` justifies: do not let `/harness:deep-review` reshape code that a fix is about to rewrite.

---

## Phase 4 — `/harness:deep-review`

Run `deep-review.md` over the files **this run touched** (Phase 0's snapshot, not everything dirty), **pipeline mode** — its own contract lists `/quick-change` Phase 4 as a pipeline caller, so it applies findings rather than only reporting them: it applies its high-conviction structural findings and records anything needing a human call under `NEEDS_HUMAN`. Structure only — file size, spaghetti growth, layering, comment noise, code-judo simplifications. It is not a bug hunt.
...
`/harness:deep-review`: <N poprawek zastosowanych | czysto>
Gates: <✅ zielone — komendy | ❌ + output>

NEEDS_HUMAN: <z `/harness:deep-review`, albo "brak">
Poza zakresem, zauważone: <albo "brak">
<Uwaga: Phase 0 zgłosiła <przesłankę>, pojechaliśmy przez /quick-change na Twoją decyzję.>

Werdykt: <✅ gotowe do `/harness:commit` (Claude Code) · `$commit` (Codex) | ❌ eskalacja: co blokuje>
```

Then stop. **`/quick-change` never commits** — the tree is left ready for `/harness:commit`.
...
- **A fundamental objection stops the lane** — it goes to the user with a route to `/harness:brainstorm` / `/harness:plan-feature`, and is never patched around inline.
