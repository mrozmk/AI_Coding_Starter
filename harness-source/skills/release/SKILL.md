---
name: release
description: Create a version release — update CHANGELOG only if the project keeps one, bump the detected manifest, offer a documentation sync, commit only when files changed, tag HEAD, offer to push.
wrapper-description: Create a version release (commit + tag)
---

# /release — create a new version release (commit + tag)

**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `git` must be `true`; otherwise stop: `git group disabled in the project profile`. On Codex this skill is `$release`; `/release` below names this skill on either host.
...
- If the range contains any `feat`, `feat!`, or `BREAKING CHANGE` — i.e. user-facing surface likely changed — **offer** to sync docs: "This release includes feature/breaking changes. Run documentation-manager to sync README / `docs/` before tagging? (yes/no)"
  - If **yes**: gather the changed files (`git diff --name-only <last-tag>..HEAD`) and on Claude Code spawn the `harness:documentation-manager` agent with that list and the instruction to sync docs narrowly to what changed (it leaves `.agents/memory/` alone); on Codex, which has no agents, perform that narrow sync yourself in this conversation under the same instruction. Its edits are staged with the rest in Step 5.
...
Use `rg --files -g <pattern>` (never `grep`) for presence checks, per the project's CLAUDE.md → Search Commands rule.
...
- Stage them with **explicit paths only** — `git add <path>` per file. NEVER `git add .` / `git add -A` / `git add -u`; they sweep up unrelated work from parallel sessions (same rule as `/harness:commit` (Claude Code) · `$commit` (Codex)). Skip `.env`, credentials, large binaries.
- Run `git add` and `git commit` as **separate** commands, not chained with `&&` — the `guard-commit` hook (plugin `hooks/hooks.json`; legacy `guard-commit.sh`) inspects the staged set at commit time and blocks a commit whose stage looks empty, which a single `add && commit` line can trip.
- Commit:
  ```
...
- If yes: run `/harness:push` (Claude Code) · `$push` (Codex) — it will detect the current branch, push commits, then push tags automatically when new local tags are detected.
- If no: inform the user: "Run `/harness:push` (Claude Code) · `$push` (Codex) when ready to publish the release."
...
- NEVER `git add .` / `git add -A` / `git add -u` in Step 5 — stage explicit paths only, and run `git add` and `git commit` as separate commands (per the `guard-commit` hook)
- ALWAYS use `rg` for file/content searches — never `grep` or `find` (per CLAUDE.md → Search Commands)
