---
name: gates-verify-implementation
description: Verify implementation correctness and code quality after executing a plan — task completion, quality gates, deep semantic review, design compliance. Report-only: it never modifies code.
argument-hint: "[plan-name]"
---

# /harness:gates-verify-implementation — Verify Plan Execution & Code Quality

**Prerequisite:** `prime` ran in this session. **Group:** `node <plugin_root>/scripts/profile.mjs groups --project-root <project_root>` → `execution` must be `true`; otherwise stop: `execution group disabled in the project profile`. On Codex this skill is `$gates-verify-implementation`; `/harness:gates-verify-implementation` (Claude Code) · `$gates-verify-implementation` (Codex) below names this skill.

**Input:** a plan name or a path to a plan file. Empty → the most recently modified plan in `.agents/plans/active/`, then `done/`. Step 1 below resolves it.

Run after `/harness:execute` (Claude Code) · `$execute` (Codex) completes a plan. Validates task completion, runs quality gates, performs deep semantic code review, and checks design compliance. Do NOT modify code — only report findings.
...
1. If the input looks like a plan name (e.g. `phase-3b-ui-hero-score`) → resolve the file under `.agents/plans/active/` or `.agents/plans/done/`.
...
> Warn only, deliberately: this command is read-only. `/harness:deep-review` (Claude Code) · `$deep-review` (Codex) standard 8 is what deletes the noise, and `guard-comments.sh` is what nudges at write time. See CLAUDE.md → Style & Conventions.
...
- If the plan required **E2E** — was it validated? Use the project's test runner, MCP Playwright via `/harness:test-e2e` (Claude Code) · `$test-e2e` (Codex) if available, or document a manual check.
