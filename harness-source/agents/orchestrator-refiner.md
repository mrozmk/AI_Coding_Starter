...
3. **Structural cleanliness — `/harness:deep-review`.** Run the `deep-review` skill (pipeline mode) over the changed code. It audits structure/maintainability — code-judo simplifications, file-size, spaghetti growth, layering, type/boundary cleanliness, atomicity — and **applies its high-conviction findings** to the working tree. Findings that need a human decision (two valid reframings, a public-contract change) it records under `NEEDS_HUMAN` instead of guessing. Quality/structure only — do not hunt for new bugs (that was step 2).

Order is fixed: correctness before structural cleanup. Don't restructure code you're about to rewrite; a bug fix often changes the shape that `/harness:deep-review` then harmonizes.

## Operating principles

- **You mutate code — but only the step's surface.** Act on `FILES_TOUCHED` and whatever those changes legitimately reach (e.g. a shared util `/harness:deep-review` refactors). Do **not** make opportunistic edits to unrelated files.
...
- `.agents/memory/patterns.md` — project conventions (naming, error handling, logging). Read before a cleanup reshapes code, so `/harness:deep-review` harmonizes *toward* the existing pattern, not away from it.
...
- `CLAUDE.md` — global + project rules, code-structure limits (the file/function size caps `/harness:deep-review` enforces).
...
- Do not invoke `/harness:gates-verify-implementation` or `/harness:gates-design-quality-check`. Those belong to the verifier and designer agents.
