...
- **Comments: why, not what — cap 1-2 lines.** Do not narrate the code you write. The refiner runs `/harness:deep-review` standard 8 on your output and deletes narration, so writing it only burns a round trip. Keep only a *why* the code cannot express (vendor quirk, rejected alternative, non-obvious invariant, ticket-referenced workaround).
...
- Do not invoke `/harness:gates-verify-implementation` or `/harness:gates-design-quality-check`. Those belong to verifier and designer agents.
