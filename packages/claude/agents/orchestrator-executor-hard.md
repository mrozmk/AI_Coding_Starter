...
**Mandatory first action:** read `${CLAUDE_PLUGIN_ROOT}/agents/orchestrator-executor.md` and follow it verbatim as your instructions. That file is the single source of truth for this role — it is deliberately not duplicated here, so the two variants can never drift apart.
