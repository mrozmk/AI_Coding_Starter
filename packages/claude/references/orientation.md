...
> The plugin's packed orientation, written for spawned children. A Codex worker or a subagent has no slash commands and no plugin skills; it is handed these steps and told to *read and follow* them. Quick mode = sections 1-3, 6-7. It mirrors `skills/prime/SKILL.md`, which stays the source of what a priming session actually loads.
