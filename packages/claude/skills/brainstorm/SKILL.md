...
- `needs-context` → supply the missing files as `--dep` (typed: `missing-file` → add the file; `required-decision` → ask the user and record the answer in the spec; `external-fact` → verify against current docs and add it as a dependency; in hybrid context (`review.context: hybrid`) a `missing-file` item carries a `reason`: `budget` → pass it as `--priority-read <path[:offset-limit]>` in the continuation, `--dep` only when the whole file fits next to the mandatory pack; `excluded` → never supplied; `absent` → a finding about the spec, not a pack gap), run `--dry-run yes` again, show only the files **added** since the previous manifest, wait for the user's go (a host-level consent control may ask for it separately — stop with that question, never bypass it), then re-run — the orchestrator treats it as a continuation, not a new round, at most twice; after that the gaps are the user's decision.
