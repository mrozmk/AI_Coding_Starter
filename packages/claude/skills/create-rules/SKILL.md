...
**Input:** optional `--rerun` to re-derive facts even when the rules already look complete, and `--map` to force the large-codebase map branch of Phase 1. Anything else is ignored.
...
<HARD-GATE>
Every byte written into a rules file goes through `scripts/rules.mjs`. This skill discovers facts and gets them confirmed; it never edits `CLAUDE.md`, `.agents/project-rules.md` or `AGENTS.md` with its own tools, and writes no rules file and no `.agents/memory/` file before the facts-review screen in Phase 4. The map branch's scratch state under `.agents/harness-state/map/` (manifest, partition and node summaries, cursor) is the one write allowed before that screen — it is machine-local, gitignored, and never a rule or a memory file.
</HARD-GATE>
...
**Large codebase — the map branch.** Trigger: more than **400** tracked source files, or `--map`. Skipped when `.agents/memory/architecture.md` is `status: populated` and neither `--rerun` nor `--map` was given. The map is a **proposal** until Phase 7; its intermediate state lives in `.agents/harness-state/map/`.

1. **Partition.** `git ls-files` plus untracked-not-ignored files, noise dropped by extension and directory, grouped by top-level directory, each partition capped at **25 files and 200 KB of source**; a coarse import scan gives in-degree, and the highest-in-degree partitions are marked `core`. Write `harness-state/map/manifest.json` — the partitions, a per-file SHA-256, the analysis procedure version (`map-1`) and the options this run was given — **before** any analysis.
2. **Checkpoint screen.** One stop: the partition list, the noise that was skipped, an optional ignore list to add, and the execution model plus the caps below. This is the "confirm before spending the budget" stop; the run does not start without it.
3. **Partition summaries.** One bounded summary per partition, **≤ 40 lines** (purpose, entry points, key files, external dependencies, patterns), persisted to `harness-state/map/<partition>.md` and keyed by the partition's **content hash** (normalized paths + per-file SHA-256) together with the procedure version.
4. **Reduction tree.** Reduce the leaf summaries with bounded fan-in at every level: a node takes at most **16 children** or **60 KB** of input and emits **≤ 60 lines**; levels are added until a single root remains. A node is recomputed whenever any child's hash changed.
5. **Root = the proposal.** The root summary set is the map proposal. The aggregator never re-reads source.

**Execution model — the context boundary is explicit per host.**

- **Claude Code:** every partition summary and every reduction node runs in an **isolated read-only subagent** bounded by the caps above; the parent session sees only the persisted outputs.
- **Codex:** there are no subagents, so the boundary is the **session**. One `$create-rules --map` invocation processes at most **10 partitions (≤ 1 MB of source)** or **8 reduction nodes (≤ 480 KB of summary input)**, whichever limit is reached first and regardless of level boundaries. The manifest carries a cursor — `next: {level, node}` or `next: {partition}` — so a level wider than one session's budget is checkpointed mid-level, and the run ends with the line `k of n partitions / m of p nodes done — run $create-rules --map again to continue`. Each continuation is a fresh session that reads only the manifest and the summaries the next node needs — never the previous session's source and never another node's input. The budget is against a single session's context, tool output included, not a cumulative per-run byte count.

**Invalidation.** A partition is re-summarised when its content hash or the procedure version changed; `--rerun` forces recomputation of everything.
...
Ran the map branch? Add to the same screen: the map proposal, coverage as `analyzed / total` partitions, the `unanalyzed` list, and the manifest hash.

No rules or memory file is written before this screen (the map scratch is excepted — see Phase 1). Detected values are confirmed, never assumed — a wrong validation command discovered three sessions later costs more than one question now.
...
Write `.agents/memory/architecture.md` with frontmatter `status: populated`, `populated_by: create-rules`, and a description naming it the cross-cutting repository map. When the map branch ran, the frontmatter also carries `coverage:`, `unanalyzed:` and `map_manifest:` (the manifest hash) — this phase is the single durable write of the map. Body: **source layout** (the tree, two levels), **module roles** (a `Path | Responsibility` row per significant directory), **naming rules** (files, symbols, tests), **critical conventions** — the non-obvious rules that decide *where* new code goes.
...
- **map:** `skipped` | `<k/n partitions, m levels>` | `checkpoint <cursor>`.
