...
3. **Planning flow:** `$brainstorm` → spec (Draft) → independent review → the user's approval (`scripts/approval.mjs` receipt) → optionally `$plan-feature` (only when the profile says `planning.after_brainstorm = plan-feature` and the user did not say stop). No skill in this plugin writes application code, commits, pushes or deploys.
