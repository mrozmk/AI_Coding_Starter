...
- `channel` present (Claude Code only; independent of `ok`) → compare `installed_version` (this project's registry entry — what an update changes) and `expected_version` (the pin) and report **one** of three states, never two:
  - `behind: true` → the listing is ahead of the installation. Warning: `harness <listing_version> listed on the <marketplace> channel, project installed <installed_version> — <the two channel.update commands, verbatim>`. Print the commands **only** from `channel.update`: the `--scope` comes from that entry, and when the entry is missing or ambiguous the field is absent — then say what the probe reported (`channel.probe`, `channel.reason`) and print no command at all.
  - `behind: false` **and** `installed_version` ≠ `expected_version` → the update already ran and the project has not adopted it. Warning: `installation serves harness <installed_version>, this project runs <expected_version> — restart the session if it still loaded the old bytes, then bind it`. Never print the update commands here; they are done. This is the window between step 1 and step 3 of `installation.md → Update (project)`, and it is **not** covered by the `upgrade` bullet above: that one compares the loaded package with the bound one, so a session still running the old bytes reaches `ok: true` with no `upgrade` at all.
  - otherwise (`behind: false` and the versions agree, `comparable: false`, or a `probe` field) → no Warning.
  The listing is a local copy refreshed at `listing_updated_utc`: equality proves it is not ahead, never that it matches the remote channel. Codex gets no `channel` — the host publishes no channel file to read.
