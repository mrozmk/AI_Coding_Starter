...
- `channel` present (Claude Code only; independent of `ok`) → `channel.behind: true` means the local marketplace copy lists a newer release than this project runs. Warning: `harness <listing_version> listed on the <marketplace> channel, project runs <expected_version> — <the two channel.update commands, verbatim>`, then `prime` reports the exact bind command on the next run, once the host has actually loaded the new bytes. Print the commands **only** from `channel.update`: the `--scope` comes from this project's registry entry, and when that entry is missing or ambiguous the field is absent — then say what the probe reported (`channel.probe`, `channel.reason`) and print no command at all. `behind: false`, `comparable: false` or a `probe` field is not a Warning. The listing is a local copy refreshed at `listing_updated_utc`: equality with the pin proves it is not ahead, never that it matches the remote channel. Codex gets no `channel` — the host publishes no channel file to read.
