...
Generate the variants in parallel (spawn sub-agents via the Agent tool with separate calls in a single message — see how `/harness:design` (Claude Code) · `$design` (Codex) fans out three variants). Each variant must be a **genuinely different** interface for the same deepened module, driven by a different design constraint:
