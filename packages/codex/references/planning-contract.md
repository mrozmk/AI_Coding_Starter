...
- Approval identity is **in the spec**: `scripts/approval.mjs stamp` applies only the declared metadata transition (`**Status:** Approved`, `**Approval:** approved by the user on <date> · decision: <ref> · body-sha256 \`<hash>\``). The hash covers the *canonical* form of the file — the `**Approval:**` line removed, `**Status:**` pinned to Approved — so a Draft and its stamped form hash identically and the file can name its own bytes without a second artifact. `plan-feature` runs `approval.mjs verify`: Draft, a missing or hand-edited approval line, or any byte changed after approval → refused. An editorial edit after approval needs a new stamp (user re-approves the bytes), not automatically another substantive review.
