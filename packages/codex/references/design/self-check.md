...
`the `gates-design-quality-check` skill`. Deep rule references:
`references/design/emil-design.md` (motion), `references/design/redesign.md` (anti-AI),
`references/design/dials.md` (variance/density). The token set is the **project's detected
...
internally consistent against the "hand" it chose (`references/design/hands.md`).
