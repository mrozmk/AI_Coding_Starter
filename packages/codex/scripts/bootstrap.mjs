...
//   node scripts/bootstrap.mjs wrappers --project-root <dir> --plugin-root <dir> [--prune] [--consent yes]
...
import { assertInside, isInside, listFiles, readJson, realpathOrSelf, toPosix, writeBytes } from './lib/fsx.mjs';
import { sha256Hex } from './lib/digest.mjs';
import { STATE_DIR } from './lib/locator.mjs';
import { parseFrontmatter } from './lib/frontmatter.mjs';
import { isGeneratedWrapperFor } from './lib/wrapper.mjs';
...
export function syncWrappers({ projectRoot, pluginRoot, consent = false, prune = false }) {
  ...
  // Preflight before any mutation: listFiles never traverses a symlinked entry but does follow the
  // root it is given, so a symlinked .claude or .claude/commands would let a consented run write or
  // delete outside the project. Both are checked: a symlinked .claude whose target has no commands/
  // leaves dest non-existent, and an unresolvable path resolves back inside the root.
  const root = realpathOrSelf(projectRoot);
  const dest = path.join(root, '.claude/commands');
  const escape = [path.join(root, '.claude'), dest].find((p) => !isInside(root, p));
  const existing = listFiles(dest);
  if (escape || existing.symlinks.length) {
    return { ok: false, reason: `${toPosix(path.relative(root, escape ?? dest))} escapes the project root — refusing to copy or delete`, files: [] };
  }
  const files = [];
  const shipped = listFiles(src).files.filter((f) => f.endsWith('.md'));
  const current = new Set(shipped.map((f) => f.slice(0, -3)));
  ...
  for (const name of shipped) {
    ...
    if (status !== 'kept' && consent) { assertInside(root, target, 'wrapper copy'); fs.mkdirSync(path.dirname(target), { recursive: true }); fs.writeFileSync(target, text); }
    files.push({ path: toPosix(path.relative(root, target)), status, replaces_local_edit: before !== null && before !== text && !before.includes('generated by scripts/build-harness.mjs') });
  }
  // An orphan is a file that is self-evidently a generated wrapper for its own command path and that
  // no current wrapper claims. Reported in every preview; deleted only under --prune --consent yes.
  for (const name of existing.files.filter((f) => f.endsWith('.md'))) {
    const command = name.slice(0, -3);
    if (current.has(command)) continue;
    const abs = path.join(dest, name);
    if (!isGeneratedWrapperFor(command, fs.readFileSync(abs, 'utf8'))) continue;
    if (prune && consent) { assertInside(root, abs, 'wrapper prune'); fs.rmSync(abs); }
    files.push({ path: toPosix(path.relative(root, abs)), status: 'orphaned' });
  }
  return { ok: true, written: consent, pruned: Boolean(prune && consent), files };
}
...
function main() {
  ...
  if (cmd === 'wrappers') {
    // `--prune` is a bare flag, so parseArgv yields `true`; a `=== 'yes'` test alone would make the
    // documented form a silent no-op.
    const prune = opts.prune === true || opts.prune === 'yes';
    const res = syncWrappers({ projectRoot, pluginRoot: requireOpt(opts, 'plugin-root'), consent: opts.consent === 'yes', prune });
    console.log(JSON.stringify(res, null, 2));
    if (opts.consent !== 'yes') {
      const orphans = (res.files ?? []).filter((f) => f.status === 'orphaned').map((f) => f.path);
      const note = orphans.length ? `; --prune --consent yes would delete ${orphans.length} orphaned wrapper(s): ${orphans.join(', ')}` : '';
      console.error(`preview only (written: false) — a file marked replaces_local_edit holds project edits; re-run with --consent yes after the user approved the list${note}`);
    }
    return;
  }
  if (cmd === 'swap') {
