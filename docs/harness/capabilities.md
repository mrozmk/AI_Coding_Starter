...
| `create-rules` — codebase discovery → budget condensation → facts review → `rules.mjs render` / brownfield `fill` → regenerated `architecture.md`; greenfield fill refused with a reason | plugin | plugin | implemented | `rules.test.mjs` (11 fill cases), `skills.test.mjs` |
