...
## Migrate 0.4.0 → 0.5.0 (downstream, by hand)

0.5.0 adds one skill and one script operation. **Nothing is deleted and no file of yours is touched** — this upgrade is additive, so a project that does nothing keeps working.

**1. The new skill.** `create-rules` derives project rules and the repository map from the code: it detects the stack and layout, condenses the result to the always-loaded budget (≤165 lines / ≤9 500 characters), presents every inferred fact with the file it came from, and then delegates every rules write to `scripts/rules.mjs`. It also regenerates `.agents/memory/architecture.md`, which nothing in the plugin produced before. It is namespaced, so it has **no wrapper**: invoke it as `/harness:create-rules` (Claude Code) or `$create-rules` (Codex).

**2. The new script operation — `rules.mjs fill`.** Until now no component could put a discovered fact into a rules file that already existed: `applyRules` is absent-only in both branches, so a `CLAUDE.md` left with `{typecheck-command}` and an unfilled `### Branch model` stayed that way forever, and `prime` kept reporting it `incomplete`. `fill` resolves exactly the unresolved spans — a named placeholder, a `Validation` fence whose command is still a placeholder, a branch-model field that is placeholder-valued **or absent altogether** — with a diff preview, a per-field `--set` for an approved overwrite, and a pre-write re-check that refuses anything which would worsen the legacy contract. A value that is already resolved is never changed without `--set`.

**`fill` is brownfield-only in this release.** A greenfield project's rules are generated from facts, and carrying a fill into the compatibility `CLAUDE.md` needs field→span locators `MIRRORED` does not have (no `Trunk` / `Integration` entries of its own; five template fields absent). Rather than leave that copy stale while `authority` still reported `ready`, `fill` refuses `greenfield` with that reason. Greenfield support is a recorded follow-up, not an oversight.

**3. Two documentation claims were false and are now corrected.** `setup-start` Phase B step 2 promised to fill placeholder branch-model fields — the code never could, and Screen 2b is skipped for exactly the project shape that needs it. It no longer claims to, and it now lets an `incomplete` rules result continue through the memory seed and the binding before routing you to `create-rules`; blocking there deadlocked a brownfield bootstrap, because the only skill that can fill those fields needs the binding that phase had not reached. Five in-plugin citations of the legacy `/setup:create-CLAUDE_MD` now name `create-rules` instead.

**4. The legacy command stays.** `.claude/commands/setup/create-CLAUDE_MD.md` is **not** removed and its inventory class is still `deferred`. Two gates are open: no installed-host run of `create-rules` has happened (so `capabilities.md` says `implemented`, never `verified`), and the README / LICENSE swap it also carries has no owner in the plugin — that swap only makes sense in a clone of the starter. The file gains a header pointer naming what moved. For one release both answer to the same job; that is the intended transitional state.
