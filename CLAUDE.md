...
- **AI git policy:** AI may run **non-destructive** git operations — `status`, `diff`, `log`, `add`, `commit`, `push`, `pull`, `fetch`, `stash`, `tag`, `describe`, `rev-parse`, `ls-remote`, `remote get-url`. **Destructive operations are denied** in [.claude/settings.json](.claude/settings.json): `push --force`, `push --force-with-lease`, `push -f`, `reset --hard`, `clean -f*`, `checkout -- *`, `restore .`, `rm`, `branch -D`/`-d`, `rebase`, `merge --no-ff`/`--squash`, `cherry-pick`, `config`, `remote add/remove/set-url`, `reflog expire`, `gc --prune=now`. `git merge --ff-only` is allowed and `git revert` is allowed (it only adds a new commit); a bare `git merge` and `git reset` (soft/mixed) are **not** auto-allowed either — they prompt. When an AI agent needs any of these, it must stop and ask the human.
